...
public final class MainHandler
{
    ...
    public static void listBlockedCommands(Player executor, Chunk chunk)
    {
        final ChunkHandler CHUNK_HANDLE = ClaimChunk.getInstance().getChunkHandler();

        if (CHUNK_HANDLE.getOwner(chunk) == null)
        {
            Utils.msg(executor, Config.infoColor() + "&6&l[!]&r &6This chunk isn't claimed!");
            return;
        }

        final String commands = ClaimChunk.getInstance().getChunkHandler().getBlockedCommands(chunk);

        if (commands == null || commands.split(",").length == 0)
        {
            Utils.msg(executor, Config.infoColor() + "&6&l[!]&r &6No Blocked Commands!");
            return;
        }

        if (commands.split(",")[0].length() > 2)
        {
            ClaimChunk.getInstance().getConfig().getStringList("disabledCommands.header").forEach(l -> Utils.msg(executor, l));
        }
        else
        {
            Utils.msg(executor, Config.infoColor() + "&6&l[!]&r &6No Blocked Commands!");
        ...
        for (String command : commands.split(","))
        {
            if (command.length() > 2)
            {
                Utils.msg(executor, ClaimChunk.getInstance().getConfig().getString("disabledCommands.concat") + command);
            }
        }
        if (commands.split(",")[0].length() > 2)
        {
            ClaimChunk.getInstance().getConfig().getStringList("disabledCommands.footer").forEach(l -> Utils.msg(executor, l));
        }
    }

    public static void removeBlockedCommands(Player player, Chunk chunk, String commands)
    {
        final ChunkHandler CHUNK_HANDLE = ClaimChunk.getInstance().getChunkHandler();

        if (!CHUNK_HANDLE.isOwner(chunk, player))
        {
            return;
        }

        Utils.msg(player, "&6&l[!]&r &6Removed Commands: " + commands);
        CHUNK_HANDLE.removeBlockedCommands(chunk, commands);
    }

    public static void addBlockedCommands(Player player, Chunk chunk, String commands)
    {
        final ChunkHandler CHUNK_HANDLE = ClaimChunk.getInstance().getChunkHandler();
        ...
        final List<String> oldCommands = new ArrayList<>(CHUNK_HANDLE.getBlockedCommands(chunk) != null ? Arrays.asList(CHUNK_HANDLE.getBlockedCommands(chunk).split(",")) : Collections.singletonList(""));
        String newCommands = "";
        for (String cmd : cmds)
        {
            if (!oldCommands.contains(cmd))
            {
                boolean contains = Arrays.stream(newCommands.split(",")).anyMatch(c -> c.equalsIgnoreCase(cmd));
                if (!contains)
                {
        ...
        Utils.msg(player, Config.infoColor() + "&6&l[!]&r &6Added Commands: " + newCommands);
        CHUNK_HANDLE.addBlockedCommands(chunk, newCommands);
    }
